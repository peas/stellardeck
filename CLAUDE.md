...
- **Autoflow rule: bare images get auto-positioned.** Today `![](src)` (no modifier) inside a slide with text content falls into a generic inline column layout. Should instead: (a) alternate `right`/`left` based on the previous bare image's resolved side, so a deck with several `![]()` slides gets natural rhythm without the user thinking about it; (b) measure the image aspect ratio (vertical vs horizontal) at render time and pick the layout — vertical images favor split, horizontal/landscape images can become a large centered hero with the text below. Trigger: ≥1 bare image (`![](src)`, no `right/left/fit/inline/filtered/bg`) on a slide with text.
- **Autoflow rule: bare image + text on a slide should pick a layout.** Related to the above. The current "image fills almost all the slide except for the short caption" pattern is missing — useful for closing slides, image-only-with-credit, etc. Today this case lands in `deckset-inline-single` flow which doesn't lay out, just stacks vertically.
- **Autoflow rule: rescue boring "headline + 2-3 short bullets" slides.** Open question for the design: a slide with one headline and 2-3 short bullets renders as title + bulleted list, which feels flat in the middle of a deck. Possible directions: (a) auto-promote bullets to a 2-3 column card grid; (b) auto-apply alternating-colors per bullet; (c) auto-`#[fit]` the headline + render bullets as horizontal pills under it; (d) detect this shape and pick from a small palette of layouts to break monotony. Needs design work — sketch 2-3 layouts on a real deck before committing to one.
- **Bug: closing slide with `![](image)` + text + `![qr](url)` overflows.** The parser only treats an image as background when the slide contains nothing but media (`isMediaOnly` check on every line). Any text/h1/h4/QR on the same slide demotes the image to inline, so a "background image + heading + contact + QR" closing collapses into a vertical column that overflows. Fix: detect "first non-empty line is a bare `![](src)` and there's other content below" → emit `<section data-background-image="src">` instead of inline. Mirrors Deckset's actual behavior. The same fix would also let `![filtered]` work on closing slides. Until then, workaround: split the closing into 2 slides (image+heading, then qr+contact), or use `![right fit]` for split layout.
