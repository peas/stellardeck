...
<script is:inline define:vars={{ markdown: undefined }}>
  // markdown passed via a JSON script element (not define:vars)
</script>
```

For Astro, pass markdown via a `<script type="application/json">` element rather than `define:vars` — this avoids escaping issues with backticks and special characters in markdown content.
